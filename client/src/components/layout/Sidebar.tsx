...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div
        className={`h-screen w-80 overflow-y-hidden absolute left-0 top-0 z-20 transition-all duration-300 lg:static ${
          expanded
      ...
      >
        <div className="flex flex-col ">
          <div className="flex items-center lg:justify-center px-2 pt-8">
          ...
          {sidebarMenu.map((section) => (
            <div key={section.title} className="mt-8 px-8">
              <h6 className="font-semibold text-gray-500 text-sm  dark:text-white mb-2">
                {section.title}
              </h6>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        isActive
                          ? "flex items-center py-2 px-3 gap-3 dark:hover:text-black dark:text-black bg-[#FFd88d] hover:bg-[#FFD88D]  ring-0 ring-[#e2e8f0] dark:ring-slate-700 transition duration-300"
                          : "flex items-center py-2 px-3 gap-3 dark:text-white dark:hover:text-black hover:bg-[#FFD88D] hover:ring-0 ring-1 ring-[#e2e8f0] dark:ring-slate-700 transition duration-300"
                      }
                    >
